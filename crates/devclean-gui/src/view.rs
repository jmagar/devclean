use crate::{
    ACCENT, ACCENT_DARK, AMBER, BG, BORDER, DevcleanApp, FINDINGS_PAGE_SIZE, MUTED, Page, RED,
    SURFACE, SURFACE_RAISED, ScanState, TEXT, VIOLET,
};
use devclean::report::ScanReportV1;
use devclean_core::{AdvisoryCandidate, CoverageStatus, Tier};
use devclean_gui::{TierFilter, candidate_location, category_label, format_bytes};
use gpui::{
    AnyElement, Context, FontWeight, MouseButton, Render, SharedString, Window, div, prelude::*,
    px, rgb,
};
...
impl DevcleanApp {
    ...
    fn header(&self, title: &str, subtitle: &str, cx: &Context<Self>) -> AnyElement {
        let scanning = matches!(self.scan_state, ScanState::Scanning | ScanState::Cancelling);
        div()
            .h(px(82.))
            .flex_none()
            .px_7()
    ...
    fn hero(&self, has_report: bool, cx: &Context<Self>) -> AnyElement {
        let (title, body) = match &self.scan_state {
            ...
            ScanState::Cancelling => ("Cancelling", "Stopping and reaping the scanner process"),
            ScanState::Error(_) => (
    ...
    fn findings(&self, cx: &Context<Self>) -> AnyElement {
        let candidates = &self.filtered_indices;
        let page_count = candidates.len().div_ceil(FINDINGS_PAGE_SIZE).max(1);
        let page = self.findings_page.min(page_count - 1);
        let start = page * FINDINGS_PAGE_SIZE;
        let list = candidates.iter().skip(start).take(FINDINGS_PAGE_SIZE).fold(
            div().flex().flex_col().gap_2(),
            |list, actual_index| {
                let candidate = &self
                    .report
                    .as_ref()
                    .expect("indices require report")
        ...
        let pagination = div()
            ...
            .child(format!(
                "Showing {}–{} of {}",
                if candidates.is_empty() { 0 } else { start + 1 },
                (start + FINDINGS_PAGE_SIZE).min(candidates.len()),
                candidates.len()
            ))
            .child(
                div()
                    ...
                    .items_center()
                    .gap_2()
                    .child(self.page_button("Previous", page > 0, false, cx))
                    .child(format!("Page {} of {}", page + 1, page_count))
                    .child(self.page_button("Next", page + 1 < page_count, true, cx)),
            );
        div()
            .size_full()
            .flex()
    ...
    fn page_button(
        ...
        let button = div()
            .id(SharedString::from(format!("findings-page-{label}")))
            .px_3()
            .py_1()
            .rounded_lg()
            .border_1()
        ...
        if enabled {
            button
                ...
                .on_click(cx.listener(move |this, _, _, cx| {
                    if forward {
                        this.findings_page = this.findings_page.saturating_add(1);
                    } else {
                        this.findings_page = this.findings_page.saturating_sub(1);
                    }
                    this.selected = None;
                    cx.notify();
                }))
